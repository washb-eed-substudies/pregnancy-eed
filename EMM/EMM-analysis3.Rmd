...
for(i in 1:nrow(H3_models)){
  ...
  if(grepl("_def", H3_models$X[i])){
    preds <- predict_gam_emm(fit=H3_models$fit[i][[1]], d=H3_models$dat[i][[1]], quantile_diff=c(0.25,0.75), Xvar=H3_models$X[i], Yvar=H3_models$Y[i], binaryX=T)
  }else{
    preds <- predict_gam_emm(fit=H3_models$fit[i][[1]], d=H3_models$dat[i][[1]], quantile_diff=c(0.25,0.75), Xvar=H3_models$X[i], Yvar=H3_models$Y[i])
  }
  gamm_diff_res <- data.frame(V=H3_models$V[i] , preds$res) %>% mutate(int.Pval = c(NA, H3_models$int.p[[i]]))
  
  H3_res <-  bind_rows(H3_res , gamm_diff_res)
}
...
H3_res <- H3_res %>% mutate(BH.Pval=p.adjust(Pval, method="BH"),
                            BH.Pval.int=p.adjust(int.Pval, method="BH")) 

#Save models
saveRDS(H3_models, here("models/emm_tr3.RDS"))

#Save results
saveRDS(H3_res, here("results/unadjusted/emm_tr_res3.RDS"))

#Save plot data
saveRDS(H3_plot_data, here("figure-data/em_unadj_spline_data3.RDS"))
...
for(i in 1:nrow(H3_adj_models)){
  ...
  if(grepl("_def", H3_adj_models$X[i])){
    preds <- predict_gam_emm(fit=H3_adj_models$fit[i][[1]], d=H3_adj_models$dat[i][[1]], quantile_diff=c(0.25,0.75), Xvar=H3_adj_models$X[i], Yvar=H3_adj_models$Y[i], binaryX=T)
  }else{
    preds <- predict_gam_emm(fit=H3_adj_models$fit[i][[1]], d=H3_adj_models$dat[i][[1]], quantile_diff=c(0.25,0.75), Xvar=H3_adj_models$X[i], Yvar=H3_adj_models$Y[i])
  }
  gamm_diff_res <- data.frame(V=H3_adj_models$V[i] , preds$res) %>% mutate(int.Pval = c(NA, H3_adj_models$int.p[[i]]))
  
  H3_adj_res <-  bind_rows(H3_adj_res , gamm_diff_res)
}
...
H3_adj_res <- H3_adj_res %>% mutate(BH.Pval=p.adjust(Pval, method="BH"),
                                    BH.Pval.int=p.adjust(int.Pval, method="BH")) 

#Save results
saveRDS(H3_adj_res, here("results/adjusted/emm_tr_adj_res3.RDS"))

#Save plot data
saveRDS(H3_adj_plot_data, here("figure-data/emm_tr_adj_spline.data3.RDS"))
...
emm <- readRDS(here('results/adjusted/emm_tr_adj_res3.RDS'))
...
```{r}
tbl3 <- subgroup_tbl("Maternal Estriol", 
                     c("Ln Estriol (ng/mL)"),
                     c("Ln Lactulose Age 3 months (mmol/L)", "Ln Mannitol Age 3 months (mmol/L)", "Ln MPO Age 3 months (ng/mL)", "Ln A1AT Age 3 months(mg/g)", "Ln NEO Age 3 months (nmol/L)", "Ln Lactulose Age 14 months (mmol/L)", "Ln Mannitol Age 14 months (mmol/L)", "Ln MPO Age 14 months (ng/mL)", "Ln A1AT Age 14 months(mg/g)", "Ln NEO Age 14 months (nmol/L)", "Ln REG1β Age 14 months (µg/mL)", "Ln Lactulose Age 28 months (mmol/L)", "Ln Mannitol Age 28 months (mmol/L)", "Ln MPO Age 28 months (ng/mL)", "Ln A1AT Age 28 months(mg/g)", "Ln NEO Age 28 months (nmol/L)"), 
                     c("Arm"), 
                     c("ln_preg_estri"),
...
save_as_docx("EMM Table: Effect modification of maternal estriol and child environmental enteric dysfunction by treatment arm" = tbl3,
            path='/Users/xianshengyan/pregnancy-eed/EMM/EMM3.docx',
